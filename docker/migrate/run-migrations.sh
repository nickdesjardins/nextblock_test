#!/bin/sh
# Applies the NextBlock SQL migrations (mounted at /migrations) to the self-hosted Postgres in
# chronological filename order, once GoTrue has provisioned the auth schema (profiles etc. FK to
# auth.users). Idempotent: a tracking table records applied versions, so restarting the stack
# never re-runs a migration. Each file is applied in a single transaction (ON_ERROR_STOP).
set -eu
